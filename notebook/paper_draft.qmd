...
```{r}
#| label: table-variable-inventory
...
```{r}
#| label: table-typology-model-summary
...
```{r}
#| label: table-typology-cluster-sizes
...
The typology profiles are summarized in original units (@table-typology-profile-original) and visualized as a profile heatmap (@fig-typology-heatmap) and radar profiles (@fig-typology-radar). The results suggest five corridor types.
...
```{r}
#| label: table-typology-profile-original
...
The typology assignments are not evenly distributed across the four study cities (@fig-typology-spatial; @fig-typology-by-city). Dresden and Poznan show a higher percentage of Typologies 1 and 2 in central and peri-urban corridors, whereas Jablonec nad Nisou is dominated by Typology 5 in its steep valley network and Typology 4 along more sinuous forested reaches. Senica presents a more even mix of open low-access and riparian-rich corridors across its low-gradient agricultural setting. The spatial maps show that typology membership follows recognizable geomorphological and land-use patterns within each city, instead of arbitrary spatial fragmentation.
...
![Overall V3 performance by typology.](../data/intervention_optimization/fig_v3_boxplot.png){#fig-v3-boxplot}

### 3.3.2 Context-dependency of intervention priorities

Cluster-specific SHAP values show that intervention priorities differ strongly across typologies and do not follow a single uniform ranking (@table-shap-top-levers; @fig-intervention-summary). In Typologies 1 and 2, slow-mobility pathway length and impervious cover have the highest mean absolute SHAP values, indicating that both mobility infrastructure and surface sealing are important in accessible but ecologically stressed urban corridors.
...
```{r}
#| label: table-intervention-results
