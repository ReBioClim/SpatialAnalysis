...
buffer["geometry"] = buffer.geometry.buffer(100) # try 100 and 50m
...
stream_segment[["segment_id", "canopy_ratio", "geometry"]].to_file(
    "data/variable/canopy_100.gpkg", driver="GPKG")

# export buffer with canopy ratio
buffer.to_file("data/variable/stream100_buffer_with_canopy.gpkg", driver="GPKG")

# some are over 1, as the pixel size can be slightly bigger than buffer size
# so replace those bigger than 1