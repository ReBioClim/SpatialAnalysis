...
source_streams = gpd.read_file("data/stream_geometry/streams_01_extract.gpkg")
merged_stream = gpd.read_file("data/stream_geometry/streams_02_network.gpkg")

source_streams["covering"] = source_streams["covering"].astype(str).str.lower().str.strip()
raw_candidates = source_streams[
    source_streams["covering"].isin(["piped", "covered"])
][["geometry"]]
...
result["underground_ratio"] = (result["und_len"] / result["seg_len"]).clip(0, 1).fillna(0)


result[["segment200_id", "geometry", "underground_ratio"]].to_file(
