...
output_path = "data/production/variables/v2_access_poi.gpkg"
...
for city in cities:
    ...
    pp = poi_points[poi_points["city"] == city]
    pg = poi_polygons[poi_polygons["city"] == city].copy()

    s["n_entry"] = 0
    s["access"] = 0.0

    pg["geometry"] = pg.geometry.centroid if len(pg) else pg.geometry
    poi = pd.concat([pp[["geometry"]], pg[["geometry"]]], ignore_index=True)

    if len(r) == 0 or len(s) == 0 or len(poi) == 0:
        results.append(s)
        continue

    if "fclass" in r.columns:
        r = r[~r["fclass"].isin(["motorway", "trunk", "motorway_link", "trunk_link"])]
    ...
    net.set_pois(
        ...
        maxdist=max_dist,
        maxitems=max_pois,
        x_col=poi.geometry.x.values,
        y_col=poi.geometry.y.values,
    )
...
all_data["poi_access_index"] = minmax(all_data["access"].values)
all_data["POI_count"] = all_data["access"]

cols = ["segment400_id", "merged_id", "city", "POI_count", "poi_access_index", "geometry"]
